...
describe('AuthService', () => {
  ...
  it('should clear localStorage on logout', () => {
    // We CANNOT test window.location.href directly (browser restricted)
    // So we spy on the logout method and only test localStorage clearing.

    const clearSpy = spyOn(localStorage, 'clear').and.callThrough();

    // Replace navigation to avoid touching window.location
    spyOn(service as any, 'logout').and.callFake(() => {
      localStorage.clear();
    });
