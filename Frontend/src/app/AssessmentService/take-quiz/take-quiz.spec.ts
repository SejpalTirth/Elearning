...
describe('TakeQuizComponent', () => {
  ...
  beforeEach(async () => {
    ...
    mockProgress = {
      markModuleCompleted: jasmine.createSpy().and.returnValue(of({}))
    };
  ...
  it('should submit quiz and mark module completed when passed', () => {
    ...
    mockApi.submitQuiz.and.returnValue(
      of({ passed: true, submissionId: 'S999' })
    );

    component.submit();

    expect(mockApi.submitQuiz).toHaveBeenCalled();
    expect(mockProgress.markModuleCompleted).toHaveBeenCalledWith({
      userId: 'U1',
      moduleId: 55
    });

    expect(mockRouter.navigate).toHaveBeenCalledWith(
      ['/assessment/result/S999'],
      { queryParams: { moduleId: 55 } }
    );
  });
  ...
  it('should navigate to result even if not passed', () => {
    ...
    mockApi.submitQuiz.and.returnValue(
      of({ passed: false, submissionId: 'S100' })
    );
